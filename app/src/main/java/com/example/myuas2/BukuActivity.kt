...
class BukuActivity : AppCompatActivity() {
    ...
    private lateinit var bukuAdapter: Adapter
...
//    }
    ...
    private fun setupList(){
        lisNote = findViewById(R.id.list_buku)
        bukuAdapter = Adapter(arrayListOf())
        lisNote.adapter = bukuAdapter
    }
        ...
        api.data().enqueue(object : Callback<Model> {
            override fun onResponse(call: Call<Model>, response: Response<Model>) {
                if (response.isSuccessful){
                    val listData = response.body()!!.toko
                    bukuAdapter.setData( listData )
//                    listData.forEach {
